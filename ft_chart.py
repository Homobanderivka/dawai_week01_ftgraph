...
# US Dollar Index
if show_usd:
    ax1.plot(df_filtered.index, df_filtered['US_Dollar_Index'], color='crimson')
...
if show_yield:
    ax2 = ax1.twinx()
    ax2.plot(df_filtered.index, df_filtered['10Y_US_Yield'], color='navy')
...
if show_lib_day:
    ...
    if liberation_day >= start_date and liberation_day <= end_date:
        ax1.axvline(liberation_day, color='gray', linestyle='dotted', label='Liberation Day')
        st.caption("Dashed vertical line marks 'Liberation Day' (April 2, 2025)")
